import sys, os, json, csv, glob, zipfile, cv2, numpy as np, pandas as pd, re
...
class MainWindow(QMainWindow):
    ...
    def process_plate_image(self, plate_image):
        try:
            ...
            # Thresholding to increase text clarity
            _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            # OCR
            custom_config = r'--oem 3 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
            text = pytesseract.image_to_string(thresh, config=custom_config)
            text = text.strip().replace(" ", "").replace("\n", "")
            # Simple confidence approximation (Tesseract doesn't directly provide score)
            ocr_conf = 0.85  # Assumed fixed high confidence for now
            if text:
